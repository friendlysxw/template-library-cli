 ...
 * @param {*} tempList 			可选的模板列表
 * @return {*}  返回当前模板配置信息
 */
async function promptTempConfig(tempName, tempList) {

	const showList = tempList.map(v => ({ value: v.name, name: `[ ${v.name} ] ${v.desc || ''}` }));
	const tempExist = showList.some((v) => v.value == tempName);
	const tempNameWhen = (tempName === undefined) || !tempExist;
	const tempNameMessage = (tempName === undefined) ? "请选择使用的模板" :
		!tempExist ? `${infoText(tempName)} 模板不存在，请选择其它模板` : "";
	...
	const temp = tempList.find(v => v.name == tempName);
	return temp;

...
const { log, infoText, succeedLog, errorLog } = require('./utils/log');
...
async function genApp(tempConfig, genConfig) {
    ...
    if (createDir) {
        ...
        try {
            ...
            spinner.stop();
            succeedLog(`应用 ${dirNameStyle} 生成成功`);
        } catch (error) {
            spinner.stop();
            errorLog('应用生成失败');
            throw error;
        ...
        try {
            await copySync(src, dest, { overwrite: false, errorOnExist: true });
            spinner.stop();
            succeedLog(`符合模板 ${nameStyle} 的应用内容生成成功`);
        } catch (error) {
            spinner.stop();
            errorLog('应用生成失败');
            throw error;
...
async function genFile(tempConfig, genConfig) {
    ...
    try {
        await copySync(src, dest, { overwrite: false, errorOnExist: true });
        spinner.stop();
        succeedLog(`${destStyle} 生成成功`);
    } catch (error) {
        spinner.stop();
        errorLog(`${destStyle} 生成失败`);
        throw error;
    }
}


module.exports = async (tempType, tempName, newName) => {
    try {

        const clientTempConfigs = getClientConfigTemp();
        const tempList = Object.values(clientTempConfigs).filter((v) => v.type === tempType);
        const types = { app: '应用项目', file: '业务文件' };
        if (!tempList.length) {
            log(`本地暂无任何 ${infoText(types[tempType])} 类型的模板，请先配置模板再使用`);
            return;
        }

        // 获取模板配置信息
        const tempConfig = await promptTempConfig(tempName, tempList);
        // 收集生成配置信息
